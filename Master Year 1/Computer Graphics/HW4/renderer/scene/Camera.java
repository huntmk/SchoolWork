...
public class Camera
{
   ...
   */
   public void projPerspective(final double left,   final double right,
   ...
   */
   public void projPerspective(final double fovy, final double aspect, final double near)
   {
   ...
   */
   public void projOrtho(final double left,   final double right,
                         final double bottom, final double top)
   {
      this.left   = left;   // These five numbers define the camera's view volume.
      this.right  = right;  // These numbers are encoded into the camera's
   ...
   /**
      ...
      @param near    distance from the origin to the near plane
   */
   public void projOrtho(final double fovy, final double aspect, final double near)
   {
      this.top    =  near * Math.tan((Math.PI/180.0)*fovy/2.0);
      this.bottom = -top;
      this.right  =  top * aspect;
   ...
   */
   ...
   {
      final double fovy = 2.0 * (180./Math.PI) * Math.atan(top/(-n));
