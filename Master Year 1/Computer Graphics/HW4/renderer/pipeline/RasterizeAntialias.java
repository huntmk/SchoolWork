...
*/
public final class RasterizeAntialias
{
   ...
   */
   public static void rasterize(final Model model, final LineSegment ls, final FrameBuffer.Viewport vp)
   {
      ...
      final Vertex v0 = model.vertexList.get( ls.vIndex[0] );
      final Vertex v1 = model.vertexList.get( ls.vIndex[1] );

      final float[] c0 = model.colorList.get( ls.cIndex[0] ).getRGBComponents(null);
      final float[] c1 = model.colorList.get( ls.cIndex[1] ).getRGBComponents(null);
...
*/
            ...
            else // a transposed line
            {
               ...
               final int x_vp_low = y_low - 1;  // viewport coordinate
               final int x_vp_hi  = y_hi  - 1;  // viewport coordinate
               final int y_vp     = h - x;      // viewport coordinate
         ...
         else // no antialiasing
         {
            ...
            else // a transposed line
            {
               ...
               final int x_vp = (int)Math.round(y) - 1; // viewport coordinate
               final int y_vp = h - x;                  // viewport coordinate
               vp.setPixelVP(x_vp, y_vp, new Color(r, g, b));
            }
         }
      ...
      if ( ! transposedLine )
      {
         ...
         final int x_vp = (int)x1 - 1;  // viewport coordinate
         final int y_vp = h - (int)y1;  // viewport coordinate
      ...
      else
      {
         ...
         final int x_vp = (int)y1 - 1;  // viewport coordinate
         final int y_vp = h - (int)x1;  // viewport coordinate
         vp.setPixelVP(x_vp, y_vp, new Color(r1, g1, b1));
      }
   }
   ...
   */
   private static void logPixel(final int x, final double y,
   ...
   */
   private static void logPixel(final double x, final int y,
                                final float r, final float g, float b,
                                final int w, final int h)
   {
      System.err.printf(
   ...
   */
   private static void logAAPixels(final int x,  final double y,
   ...
   */
   private static void logAAPixels(final double x, final int y,
