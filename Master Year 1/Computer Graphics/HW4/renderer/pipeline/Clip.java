...
public class Clip
{
   ...
   */
   public static boolean clip(Model model, LineSegment ls)
   {
      boolean debug = Clip.debug && (Pipeline.debug || model.debug);
      ...
      if ( ! ( Math.abs( x0 ) > 1
            ...
            || Math.abs( y1 ) > 1 ) )
      {
         if (debug) System.err.println("-Trivial accept.");
         return true;
      }
      ...
      {
         if (debug) System.err.println("-Trivial delete.");
         return false;
      }
      ...
      else if (x0 > 1 || x1 > 1)  // ls crosses the line x = 1
      {
         if (x1 > 1)
         {
            if (debug) System.err.println("-Clip off v1 at x = 1.");
            // Create a new Vertex between v0 and v1.
            interpolateNewVertex(model, ls, 1, 1);
         }
         else // (x0 > 1)
         {
            if (debug) System.err.println("-Clip off v0 at x = 1.");
      ...
      else if (y0 > 1 || y1 > 1)  // ls crosses the line y = 1
      {
         if (y1 > 1)
         {
            if (debug) System.err.println("-Clip off v1 at y = 1.");
            // Create a new Vertex between v0 and v1.
            interpolateNewVertex(model, ls, 1, 3);
         }
         else // (y0 > 1)
         {
            if (debug) System.err.println("-Clip off v0 at y = 1.");
            // Create a new Vertex between v1 and v0.
   ...
   */
                                            ...
                                            int eqn_number)
   {
      ...
      if (debug) System.err.println("-Create new vertex.");

      int inside = 1 - outside;

      // Make local copies of several values.
      // "i" for "inside"
      double vix = model.vertexList.get( ls.vIndex[inside] ).x;
      double viy = model.vertexList.get( ls.vIndex[inside] ).y;
      float ci[] =  model.colorList.get( ls.cIndex[inside] ).getRGBColorComponents(null);
      // and "o" for "outside"
      double vox = model.vertexList.get( ls.vIndex[outside] ).x;
      double voy = model.vertexList.get( ls.vIndex[outside] ).y;
      float co[] =  model.colorList.get( ls.cIndex[outside] ).getRGBColorComponents(null);
      ...
      if (debug)
      {
         System.err.printf("- t = % .25f\n", t);
         System.err.printf("- <x_o,y_o> = <% .24f % .24f\n",  vox, voy);
         System.err.printf("- <x_i,y_i> = <% .24f % .24f\n",  vix, viy);
         System.err.printf("- <x,  y>   = <% .24f % .24f\n",    x,   y);
         System.err.printf("- <r_o,g_o,b_o> = <% .15f  % .15f  % .15f>\n",
                              co[0], co[1], co[2]);
         System.err.printf("- <r_i,g_i,b_i> = <% .15f  % .15f  % .15f>\n",
                              ci[0], ci[1], ci[2]);
         System.err.printf("- <r,  g,  b>   = <% .15f  % .15f  % .15f>\n",
                               r,  g,  b);
      }
   }
}
